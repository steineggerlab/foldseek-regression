...
"${FOLDSEEK}" createindex "$RESULTS/db" tmp --index-exclude 1
"${FOLDSEEK}" search "${INPUTDB}" "$RESULTS/db" "$RESULTS/results_aln" "$RESULTS/tmp" --cluster-search 1 --prefilter-mode 1 -e 10000 -s 9 --max-seqs 100 -a --remove-tmp-files 0
...
TARGET="0.986667 0.818586 0.466449"
